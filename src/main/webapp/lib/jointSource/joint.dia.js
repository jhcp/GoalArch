...
Element.prototype = {
	...
	default:
		...
		var path = "";
		for (i in this.wrapper.attrs.path)
			for (j in this.wrapper.attrs.path[i])
			path += this.wrapper.attrs.path[i][j] + ' ';
		clone = this.paper.path(path);
		//clone = paper.rect(wa.x, wa.y, wa.width+250, wa.height+50, wa.r);
	    break;
	}
	clone.attr(attrs);
        return clone;
